...
func TestVerifyValidIPOfRequest(t *testing.T) {
	paymentProvider := CreatePayfastProvider(logger, validSaleProvider, true, tmpMerchantId, "tmp-passphrase1")
...
func localTestVerifySignatureOfPostData(t *testing.T, passPhraseLeaveBlankForNone string) {
	paymentProvider := CreatePayfastProvider(logger, validSaleProvider, true, tmpMerchantId, passPhraseLeaveBlankForNone)
...
func getPrePopulatedPayfastPaymentProvider(merchantId string, saleProvider IPayfastSaleProvider) IPaymentProvider {
	paymentProvider := CreatePayfastProvider(logger, saleProvider, true, merchantId, "tmp-passphrase2")
